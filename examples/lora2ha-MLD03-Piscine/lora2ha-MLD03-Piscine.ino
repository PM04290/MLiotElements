...
{
  Wire.begin();
  Wire.beginTransmission(I2C_CODER_ADDRESS);
  if (Wire.endTransmission() == 0)
  {
    DEBUGln("Codeur détecté");
    Wire.requestFrom(I2C_CODER_ADDRESS, 1);
    if (Wire.available())
    {
      uint8_t c = Wire.read();
      if (c > 0 && c <= 99)
...
void setup()
{
  ...
  // verifiy Coder Address connected
  uint8_t co_uid = detectCoder();
  if (co_uid)
  {
    if (needConfig)
    {
      DEBUGln("New HUB address : ");
      EEPROM.put(EEP_UID, co_uid);
    } else
    {
      DEBUGln("New device address : ");
      EEPROM.put(EEP_HUBID, co_uid);
    }
    DEBUGln(co_uid);
  }
  ...
  { // blank EEPROM
    ...
    tr = {-30, 0, 5, 10};             // min:-3° max:0° sel:-0.5° div:10
    EEPROM.put(EEP_INPUTFROST_L, tr);
    tr = {0, 50, 20, 10};             // min:0° max:5.0° sel:2° div:10
    EEPROM.put(EEP_INPUTFROST_H, tr);

    tr = {3, 12, 8, 1};               // start hour from 3 to 12 (8 default)
    EEPROM.put(EEP_INPUTFILTER_START, tr);

    tr = {0, 20, 12, 1};              // refill start hour from 0 to 20h (12h default)
    EEPROM.put(EEP_INPUTREFILL_START, tr);
    tr = {0, 6, 0, 1};                // refill skip day (0 by defulat : every days)
    EEPROM.put(EEP_INPUTREFILL_DAY, tr);
    tr = {5, 600, 30, 1};             // refill duration from 5 to 600 (30 minutes default)
    EEPROM.put(EEP_INPUTREFILL_DUR, tr);
  ...
  //          0 = 27 ms
  uint8_t distance = 1;
  LoraOK = RLcomm.begin(LRfreq * 1E6, onReceive, NULL, 19, distance);
  if (LoraOK)
  {
  ...
  FilterIdle = (Binary*)ML_addElement(new Binary(PIN_I_3, CHILD_SENSOR3, F("Idle filter"), true, nullptr));
  ...
  RelayValve = (Relay*)ML_addElement(new Relay(PIN_R_1, CHILD_RELAY1, F("Refill valve")));
  ...
  RefillStartHour = (Input*)ML_addElement(new Input(CHILD_INPUTREFILL_START, EEP_INPUTREFILL_START, F("Refill start")));
  RefillDays = (Input*)ML_addElement(new Input(CHILD_INPUTREFILL_DAY, EEP_INPUTREFILL_DAY, F("Refill n/day")));
  RefillDuration = (Input*)ML_addElement(new Input(CHILD_INPUTREFILL_DUR, EEP_INPUTREFILL_DUR, F("Refill dur.")));
